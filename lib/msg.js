...
conn.downloadM = async (m, type, saveToFile) => {
    ...
    // modern memakai LID jid untuk chat dengan diri sendiri, bukan PN jid.
    // Kalau bot kirim balasan ke PN jid sendiri, pesan nyangkut "menunggu pesan
    // ini..." (pending) dan tidak pernah tampil. Di sini target yang menunjuk
    // nomor bot sendiri dialihkan ke LID jid bot.
    ...
    const _toSelfLid = (jid) => {
        try {
            if (process.env.NO_SELF_LID) return jid
            const target = String(jid || '')
            if (!target || target.endsWith('@lid')) return jid
            const tpn = target.split(':')[0].split('@')[0]
            if (!tpn) return jid
            const selfNums = _selfJidNumbers()
            const lid = String(conn.user?.lid || '')
            if (selfNums.has(tpn) && lid) {
                const pure = lid.split(':')[0]
                return pure.endsWith('@lid') ? pure : pure + '@lid'
            }
            return target
        } catch {
            return jid
    ...
    conn.sendMessage = async (jid, content, opts) => {
        ...
        try {
            ...
            if (process.env.DEBUG_SEND) {
                console.log('[DEBUG] send OK jid=' + jid + ' type=' + (content?.react ? 'react' : Object.keys(content || {})[0]) + ' ' + (Date.now() - t0) + 'ms')
            }
            return res
        } catch (e) {
            if (process.env.DEBUG_SEND) {
                console.log('[DEBUG] send FAIL jid=' + jid + ' type=' + (content?.react ? 'react' : Object.keys(content || {})[0]) + ' err=' + (e?.message || e))
            }
            throw e
        }
    }
