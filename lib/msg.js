...
conn.downloadM = async (m, type, saveToFile) => {
    ...
    conn.sendMessage = async (jid, content, opts) => {
        ...
        // TROLI KECIL DI SETIAP FITUR: kalau tidak ada quoted/lampiran di-opts,
        // sematkan kartu troli (orderMessage) sebagai quoted — sehingga hampir
        // semua balasan bot tampil seperti pesan order dari bot. Dikecualikan
        // untuk aksi sistem (reaksi, hapus, protokol, polling) & kiriman yang
        // memang sudah membawa quoted sendiri.
        try {
            const c = content && typeof content === 'object' ? content : {}
            const isAction = ['react', 'delete', 'protocol', 'pollUpdate', 'poll'].some(k => k in c)
            const tgt = String(jid).split('@')[1] || ''
            if (!isAction && Object.keys(c).length && opts && typeof opts === 'object' && !('quoted' in opts)) {
                if (tgt === 'g.us' || tgt === 's.whatsapp.net') {
                    const troli = await getTroli(conn, jid).catch(() => null)
                    if (troli) opts = { ...opts, quoted: troli }
                }
            }
        } catch {}
        const t0 = Date.now()
...
export async function smsg(conn, m) {
    ...
    if (m.key) {
        m.id = m.key?.id
        m.isBaileys = !!m.id && /^(3EB0|BAE5)/.test(m.id) && (m.id.length === 16 || m.id.length === 22 || m.id.length === 40)
