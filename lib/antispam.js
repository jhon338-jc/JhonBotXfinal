...
export async function runAntiSpam({ conn, m, body }) {
    ...
    if (!rec.warned && rec.count >= WARN_AT && rec.count <= KICK_AT) {
        ...
        try {
            ...
            console.error(rgbTag('ANTISPAM', 'Gagal kirim peringatan: ' + (e?.message || e), COLORS.error))
        }
        return false
    }
    ...
    if (!rec.kicked && rec.count > KICK_AT) {
        ...
        try {
            ...
            console.error(rgbTag('ANTISPAM', 'Gagal kick ' + m.sender + ': ' + (e?.message || e), COLORS.error))
            if (m.isBotAdmin) {
                await conn.sendMessage(m.chat, {
