...
export async function sendMediaFlow(conn, jid, { media, mimetype = '', caption = '', footer = '', buttons = [], quoted }) {
    ...
    const targetJid = conn.decodeJid(jid)
    const troli = quoted || (await getTroli(conn, targetJid).catch(() => null))
    const msg = generateWAMessageFromContent(
        targetJid,
        interactiveMsg,
        { userJid: conn.user?.id || jid, quoted: troli || undefined }
    )
