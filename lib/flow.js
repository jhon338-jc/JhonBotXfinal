...
export function mediaButtons(cmd) {
    return [
        quickReply('🔄 Kirim Ulang', `.${cmd} ulang`),
...
export async function sendMediaFlow(conn, jid, { media, mimetype = '', caption = '', footer = '', buttons = [], quoted }) {
    ...
    const header = isVideo
        ? { title: '📽️ Asupan', hasMediaAttachment: true, videoMessage: prepared.videoMessage }
        : { title: '🖼️ Random', hasMediaAttachment: true, imageMessage: prepared.imageMessage }
