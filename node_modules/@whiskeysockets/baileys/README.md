# <div align='center'>Levvleys - A WebSockets library for interacting with WhatsApp Web</div>

<div align="center">


**Levvleys** is a fast, stable, and modern interactive-feature-focused WhatsApp Web API library built on WebSocket.
...
```
npm install levvleys
```

Use the edge version (no guarantee of stability, but latest fixes + features)

```
npm install github:levvleys
```

Then import your code using:

```js
import makeWASocket from 'levvleys'
```
...
```js
import makeWASocket, { useMultiFileAuthState } from 'levvleys'
...
```js
import makeWASocket, { DisconnectReason, useMultiFileAuthState } from 'levvleys'
...
import { makeInMemoryStore, getAggregateVotesInPollMessage } from 'levvleys'
...
```js
import { jidNormalizedUser, getAggregateResponsesInEventMessage } from 'levvleys'
...
```js
import makeWASocket, { makeInMemoryStore } from 'levvleys'
                ...
                payment_settings: [
                    {
                        ...
                        pix_static_code: {
                            merchant_name: 'Levvleys',
                            key: 'example@Levvleys.com',
...
Rich Message adalah fitur UI premium pada Levvleys yang mendukung markdown, code block, table, images, video, product card, social post, reels, sources, suggestions, dan tip dalam satu pesan.
...
console.log(\`Hello \${levvi}!\`);`
    ...
    post: {
      ...
      comments_count: 0,
      shares_count: 0,
      post_url: 'https://www.levvicode.cloud/',
      post_deeplink: 'levvicode://',
    ...
    footer: '© 2025 LevviCode. All rights reserved.'
  }
}, { quoted: m })
```
...
conn.sendMessage(m.chat, {
  orderStatus: {
    ...
    title: 'LevviCode Shop',
    text: 'Silakan cek status pesanan Anda.',
    footer: 'Powered by LevviCode',
    referenceId: 'LV-001',
    status: 'PROCESSING',
    subtotalValue: 10000,
    subtotalOffset: 100,
...
await conn.sendMessage(
    id,
    {
        album: [{
        	image: {
        		url: 'https://example.com/levvleys.jpg'
        	},
        	caption: 'Hay'
        }, {
        	image: Buffer,
        	...
        	video: {
        		url: 'https://example.com/levvleys.mp4'
        	},
        	caption: 'Hay'
        }, {
        	video: Buffer,
...
import { downloadMediaMessage, getContentType } from 'levvleys'
