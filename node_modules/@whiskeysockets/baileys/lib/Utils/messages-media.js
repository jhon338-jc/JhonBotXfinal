...
const generateProfilePicture = async (mediaUpload, dimensions) => {
    ...
    let img;
    if ("sharp" in lib) {
        const sharpFunc = lib.sharp.default || lib.sharp;
        img = sharpFunc(buffer).resize(w, h).jpeg({ quality: 50 }).toBuffer();
    } else if ("jimp" in lib) {
        const jimpRead = lib.jimp.read || lib.jimp;
        const jimp = await jimpRead(buffer);
        const min = Math.min(jimp.width || jimp.getWidth(), jimp.height || jimp.getHeight());
        const cropped = jimp.crop ? jimp.crop(0, 0, min, min) : jimp;
        const resized = cropped.resize ? cropped.resize(w, h) : cropped;
        img = resized.getBufferAsync 
            ? resized.getBufferAsync("image/jpeg", { quality: 50 })
            : resized.getBuffer("image/jpeg", { quality: 50 });
    } else {
        throw new Boom("No image processing library available");
    }
