...
let handler = async (m, { conn }) => {
    ...
    try {
        const found = randomImage('pap_memek')
        if (!found) {
            return m.reply('⚠️ Folder gambar PAP MMK masih kosong!')
        }
        const buffer = fs.readFileSync(found.file)
        await saveImage(buffer)
        await conn.sendMessage(m.chat, { image: buffer, caption: '🍑 *PAP MMK RANDOM*' }, { quoted: m })
        await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })
    } catch (e) {
        console.error(rgbTag('PAPMMK', e?.message || e, COLORS.error))
        await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
        m.reply('❌ Gagal mengambil gambar PAP MMK.')
    }
}
