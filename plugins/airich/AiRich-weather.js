...
function spinning(){ref.style.transform='rotate('+spinId+'deg)';if(spinId>=360){spinId=null;cur=25+Math.floor(Math.random()*6);hi=cur+2+Math.floor(Math.random()*2);lo=cur-3-Math.floor(Math.random()*2);for(var i=0;i<temps.length;i++)temps[i]=cur+Math.floor(Math.random()*4-2);var cs=[['','Sunny'],['','Partly Cloudy'],['','Light Rain'],['','Rainy']];var c=cs[Math.floor(Math.random()*cs.length)];bigIcon.textContent=c[0];condEl.textContent=c[1];showT();renderHour();return}spinId+=45;setTimeout(spinning,90)}
...
let handler = async (m, { conn }) => {
    try {
        const html = shell({ title: 'Weather', tag: 'APP', icon: '', html: HTML, script: GAME_JS })
