...
function init(){player={x:Math.floor(cols/2),y:rows-2};vehicles=[];logs=[];score=0;over=false;
rowTypes=[];for(var r=0;r<rows;r++){if(r<waterY-1)rowTypes.push({type:'grass',speed:0});else if(r<waterY+2)rowTypes.push({type:'water',speed:0.8+Math.random()});else rowTypes.push({type:'road',speed:1+Math.random()*2})}
for(var r=waterY;r<waterY+2;r++)for(var i=0;i<2+Math.floor(Math.random()*2);i++){logs.push({x:Math.random()*W,y:r,sp:rowTypes[r].speed*(Math.random()<0.5?1:-1),w:grid*2+Math.random()*grid*2})}
...
function move(dx,dy){if(over)return;player.x+=dx;player.y+=dy;player.x=Math.max(0,Math.min(cols-1,player.x));player.y=Math.max(0,Math.min(rows-1,player.y));if(dy<0)score++;if(player.y===0){score+=10;init()}}
