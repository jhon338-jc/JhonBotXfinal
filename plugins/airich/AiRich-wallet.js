...
function renderTx(){txList.innerHTML='';txs.forEach(function(t){var d=document.createElement('div');d.style.cssText='display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;margin-bottom:6px';d.innerHTML='<div style="width:32px;height:32px;border-radius:50%;background:'+(t[0]>0?'#00b89433':'#e1705533')+';display:flex;align-items:center;justify-content:center;font-size:14px">'+(t[0]>0?'':'')+'</div><div style="flex:1;font-size:12px;font-weight:bold;color:#fff">'+t[1]+'</div><div style="font-size:13px;font-weight:bold;color:'+(t[0]>0?'#00b894':'#e17055')+'">'+(t[0]>0?'+':'-')+disp(t[0])+'</div>';txList.appendChild(d)})}
