...
<div id="app">
  <div id="vidbox" style="position:relative;border-radius:14px;overflow:hidden;background:linear-gradient(135deg,#6c5ce7,#a29bfe);aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;border:1px solid rgba(255,255,255,.14)">
    <div id="thtitle" style="font-size:20px;font-weight:bold;color:#fff;text-align:center;padding:10px">▶️ VIDEO</div>
    <div id="bigplay" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;cursor:pointer;background:rgba(10,10,20,.35)"><div style="font-size:46px">▶️</div></div>
  </div>
...
function renderSugg(){sugg.innerHTML='';vids.forEach(function(v,i){var d=document.createElement('div');d.style.cssText='display:flex;gap:10px;align-items:center;padding:8px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;cursor:pointer';d.innerHTML='<div style="width:64px;height:36px;border-radius:6px;background:'+v[1]+';display:flex;align-items:center;justify-content:center;font-size:12px">'+(i===idx?'▶️':'')+'</div><div style="flex:1"><div style="font-size:12px;font-weight:bold;color:#fff">'+v[0]+'</div><div class="muted" style="font-size:10px">'+fmt(v[2])+'</div></div>';d.onclick=function(){load(i);renderSugg()};sugg.appendChild(d)})}
function load(i){idx=i;t=0;dur=vids[i][2];tt.textContent='▶️ '+vids[i][0];vb.style.background='linear-gradient(135deg,'+vids[i][1]+',#2d2d50)';playing=false;big.style.display='flex';playBtn.textContent='PLAY';renderSugg()}
...
let handler = async (m, { conn }) => {
    ...
    try {
        const html = shell({ title: 'YouTube', tag: 'APP', icon: '▶️', html: HTML, script: GAME_JS })
