...
for(var i=0;i<9;i++){var d=document.createElement('div');d.style.cssText='display:flex;align-items:center;justify-content:center;font-size:40px;aspect-ratio:1;border-radius:12px;background:rgba(255,255,255,.06);border:2px solid rgba(255,255,255,.08);cursor:pointer;transition:all .15s';d.textContent='🕳️';d.setAttribute('data-i',i);d.onclick=function(){whack(parseInt(this.getAttribute('data-i')))};cells.push(d);grid.appendChild(d)}
updateInfo()}
...
function whack(i){if(!playing&&time<=0){startGame();return}if(!playing)return;if(!moles[i])return;clearTimeout(moles[i].timer);cells[i].textContent='💥';score++;cells[i].style.background='rgba(0,184,148,.2)';delete moles[i];setTimeout(function(){cells[i].textContent='🕳️';cells[i].style.background='rgba(255,255,255,.06)'},200);updateInfo()}
function pop(){if(!playing)return;var free=[];for(var i=0;i<9;i++)if(!moles[i])free.push(i);if(!free.length)return;var idx=free[Math.floor(Math.random()*free.length)];cells[idx].textContent='🐹';cells[idx].style.background='rgba(255,255,255,.12)';var t=setTimeout(function(){if(moles[idx]){cells[idx].textContent='🕳️';cells[idx].style.background='rgba(255,255,255,.06)';delete moles[idx]}},1000+Math.random()*500);moles[idx]={timer:t}}
function startGame(){init();playing=true;timer=setInterval(function(){time--;if(time<=0){playing=false;clearInterval(timer);for(var k in moles){clearTimeout(moles[k].timer);cells[k].textContent='🕳️';cells[k].style.background='rgba(255,255,255,.06)'}moles={}}updateInfo()},1000);var sp=setInterval(function(){if(!playing){clearInterval(sp);return}pop()},600)}
init();updateInfo();
`
