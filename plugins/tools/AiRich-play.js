// plugins/tools/AiRich-play.js
// Port SxcWaMd play2 → JhonBot Airich (.play)
...
const LRCLIB_USER_AGENT = 'JhonBot-AiRichPlay/1.0'
...
let handler = async (m, { conn, text, command }) => {
    if (!text) {
        return m.reply('Contoh:\n.' + command + ' another love tom odell')
    }
    ...
    try {
        ...
        await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })
        console.log(log('PLAY', `Lagu "${title}" dikirim ke ${m.chat} (${Math.round(html.length / 1024)} KB)`, COLORS.success))
    } catch (e) {
        console.error(log('PLAY', e?.message || e, COLORS.error))
