...
let handler = async (m, { conn, args }) => {
    ...
    if (!num || !/^\d{8,15}$/.test(num)) {
        return m.reply(' Format salah!\n\nContoh: .addprem 628xxx premium1\n\n*Paket Premium:*\nâ€¢ `.addprem 628xxx premium1` (2 hari)\nâ€¢ `.addprem 628xxx premium2` (1 minggu)\nâ€¢ `.addprem 628xxx premium3` (30 hari)')
    }
    ...
    msg += ` Aktif sejak : ${dStart}\n`
    msg += ` Berakhir   : ${dEnd}\n`
    if (before?.active && before.endDate) {
        msg += `\n _Durasi dihitung dari langganan pertama (${before.startDate ? new Date(before.startDate).toLocaleDateString('id-ID') : '-'}). Sudah pernah aktif â†’ perpanjangan otomatis._`
    }
